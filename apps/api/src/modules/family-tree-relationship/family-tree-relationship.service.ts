...
export class FamilyTreeRelationshipService {
  ...
  async getFamilyTreeRelationshipOfFamilyTree(
    ...
    const familyTreeRelationships = await this.db.query.familyTreeRelationshipsSchema.findMany({
      orderBy: asc(schema.familyTreeRelationshipsSchema.depth),
      where: eq(schema.familyTreeRelationshipsSchema.familyTreeId, familyTreeId)
    })
    ...
    // parents part
    const allParentsMixed = familyTreeRelationships.filter(data => data.depth === 0 && data.ancestorId !== data.descendantId)
    const parentMap: Map<string, {father: UserResponseType, mother: UserResponseType}> = new Map<string, {father: UserResponseType, mother: UserResponseType}>()
    ...
    for (let i = 0; i < allParentsMixed.length; i++) {
      for (let j = i; j < allParentsMixed.length; j++) {
        if (allParentsMixed[i].ancestorId === allParentsMixed[j].descendantId) {
          ...
          if (spouse1?.gender == UserGenderEnum.MALE && spouse2?.gender == UserGenderEnum.FEMALE) {
          ...
          } else if (spouse1?.gender == UserGenderEnum.FEMALE && spouse2?.gender == UserGenderEnum.MALE) {
            response.parents.push({
    ...
    // children part
    const allChildrenMixed = familyTreeRelationships.filter(data => data.depth === 1)
    ...
    for (const child of allChildrenMixed) {
      ...
      if (parent) {
        ...
        if (!registeredChild.has(child.descendantId)) {
          ...
          const registeredChildrenOfParent = childrenOfParent.get(`${parent.father.id}&${parent.mother.id}`)

          if (registeredChildrenOfParent?.length) {
            childrenOfParent.set(`${parent.father.id}&${parent.mother.id}`, [...registeredChildrenOfParent, usersInfoMap.get(child.descendantId)!])
          } else {
            childrenOfParent.set(`${parent.father.id}&${parent.mother.id}`, [usersInfoMap.get(child.descendantId)!])
          }
        }
      }
  ...
  async getFamilyTreeRelationshipUserOfFamilyTree(
    ...
    const familyTreeRelationshipData = await this.db.query.familyTreeRelationshipsSchema.findFirst({
      where: and(
  ...
  async createFamilyTreeRelationshipUserSonOfFamilyTree(
    ...
    if (!father) {
      throw new NotFoundException(
        `Father with id ${body.fatherId} not found`
      );
    }
    ...
    if (!mother) {
      throw new NotFoundException(
        `Mother with id ${body.motherId} not found`
      );
    }

    // parents relationship
    const parentFamilyTreeRelationship = await this.db.query.familyTreeRelationshipsSchema.findMany({
      where: or(
        and(
          eq(schema.familyTreeRelationshipsSchema.familyTreeId, familyTreeId),
    ...
    // create mock son
    const [son] = await this.db.insert(schema.usersSchema).values({
  ...
  async createFamilyTreeRelationshipUserDaughterOfFamilyTree(
    ...
    const [daughter] = await this.db.insert(schema.usersSchema).values({
  ...
  async createFamilyTreeRelationshipUserParentOfFamilyTree(
    ...
    await this.checkExistenceOfFamilyTree(familyTreeId)
    const parentOfTargetUser = await this.findParentOfTargetUser(familyTreeId, body.targetUserId)

    if (parentOfTargetUser) {
      throw new BadRequestException(
        `User already has parent`
      );
    }
    ...
    .values([{ gender: UserGenderEnum.MALE, name: 'UNKNOWN' }, { gender: UserGenderEnum.FEMALE, name: 'UNKNOWN' }])
    .returning();

    // self initialize
    await this.db
      .insert(schema.familyTreeRelationshipsSchema)
      .values([
        {
          familyTreeId,
          ancestorId: father.id,
    ...
    if (body.targetUserId) { // it's also called from family-tree controller when it's created
      // take children of targetUser
      const targetUserChildren = await this.db.query.familyTreeRelationshipsSchema.findMany({
        where: and(
          gte(schema.familyTreeRelationshipsSchema.depth, 1),
          eq(schema.familyTreeRelationshipsSchema.ancestorId, body.targetUserId),
          eq(schema.familyTreeRelationshipsSchema.familyTreeId, familyTreeId)
        )
      })
  ...
  async createFamilyTreeRelationshipUserSpouseOfFamilyTree(
    ...
    const existSpouses = await this.findSpouseOfTargetUser(familyTreeId, body.targetUserId)

    if (targetUser.gender === UserGenderEnum.MALE && existSpouses.length >= 4) {
      throw new BadRequestException(
        `Target user with id ${body.targetUserId} already has 4 wives`
      );
    } else if (targetUser.gender === UserGenderEnum.FEMALE && existSpouses.length >= 1) {
      throw new BadRequestException(
    ...
    // declare outer scope
    const [spouse] = await this.db.insert(schema.usersSchema).values({ gender: targetUser.gender === UserGenderEnum.MALE ? UserGenderEnum.FEMALE : UserGenderEnum.MALE, name: 'UNKNOWN' }).returning()
  ...
  async updateFamilyTreeRelationshipUserOfFamilyTree(
    ...
    if (body.email) {
      ...
      const relationship = await this.db.query.familyTreeRelationshipsSchema.findFirst({
        where: and(
      ...
      await this.db
        ...
        .where(
          and(eq(schema.familyTreeRelationshipsSchema.descendantId, userId), eq(schema.familyTreeRelationshipsSchema.familyTreeId, familyTreeId))
        )
      ...
      await this.db
        ...
        .where(
          and(eq(schema.familyTreeRelationshipsSchema.ancestorId, userId), eq(schema.familyTreeRelationshipsSchema.familyTreeId, familyTreeId))
        )
      ...
      // delete old user
      await this.db.delete(schema.usersSchema).where(eq(schema.usersSchema.id, userId))
    } else {
      ...
      // update user info
      await this.db.update(schema.usersSchema).set(body).where(eq(schema.usersSchema.id, userId))
    }
  ...
  async deleteFamilyTreeRelationshipUserOfFamilyTree(
    ...
    const userChildren = await this.findChildrenOfTargetUser(familyTreeId, userId)
    ...
    await this.db.delete(schema.usersSchema).where(and(eq(schema.usersSchema.id, userId), isNull(schema.usersSchema.email)))

    // delete relationship
    await this.db.delete(schema.familyTreeRelationshipsSchema).where(
      and(
        or(
          eq(schema.familyTreeRelationshipsSchema.ancestorId, userId),
          eq(schema.familyTreeRelationshipsSchema.descendantId, userId)
  ...
  // helpful methods
  async findRootParent(familyTreeId: string): Promise<FamilyTreeRelationshipUserArrayResponseDto> {
    const familyTreeRelationships = await this.db.query.familyTreeRelationshipsSchema.findMany({
      with: {
        ancestor: true,
      },
      orderBy: [desc(schema.familyTreeRelationshipsSchema.depth), asc(schema.familyTreeRelationshipsSchema.createdAt)],
      limit: 2,
      where: eq(schema.familyTreeRelationshipsSchema.familyTreeId, familyTreeId)
    })
  ...
  async findParentOfTargetUser(familyTreeId: string, targetUserId: string): Promise<FamilyTreeRelationshipUserArrayResponseDto> {
  ...
  async findSpouseOfTargetUser(familyTreeId: string, targetUserId: string): Promise<FamilyTreeRelationshipUserArrayResponseDto> {
  ...
  async findChildrenOfTargetUser(familyTreeId: string, targetUserId: string): Promise<FamilyTreeRelationshipUserArrayResponseDto> {
  ...
  async checkExistenceOfFamilyTree(familyTreeId: string): Promise<void> {
    ...
    if (!familyTree) {
      throw new NotFoundException(`Family tree with id ${familyTreeId} not found`)
    }
  }
}
