...
- `family_trees.visit_count` column + `public_visit_rank_idx` on `(is_public, visit_count, created_at, id)`. `GET /family-trees/public/:id` increments it; `paginateFamilyTrees` gained a `sortByVisits` flag that only swaps the `orderBy`. Owner listing untouched. Count is never selected into a response — pure sort key, so **no shared-schema or frontend change**.
- Tests: 3 unit (mocked db), 11 integration, 4 E2E.
- **Key note (design reversal — read this before "improving" it):** this shipped first as a separate `public_family_tree_visits` table (migration `0021`), on the reasoning that incrementing a counter *on* `family_trees` would force cache invalidation of the tree on every view. **That premise was false** — the later cache investigation (below) established that no single-tree route and no public-list route is cached at all, so there was nothing to protect. Folded back into a plain column; migration `0022` drops the table. Don't re-split it without a cache that actually exists.
- **Key note (the increment MUST be raw SQL):** `baseSchema.updatedAt` has `$onUpdate(() => …)`, so a drizzle `.update()` bumps `updatedAt` on every view — marking a tree modified merely for being looked at, which would mislead any client syncing on that field. `db.execute(sql\`UPDATE … SET visit_count = visit_count + 1\`)` bypasses the hook. Covered by an integration test asserting `updatedAt` is unchanged.
- **Key note (sort must be total):** order is `visit_count DESC, created_at DESC, id DESC`. Neither of the first two keys is unique — and `defaultNow()` is the *transaction* timestamp, so rows inserted in one statement share a `created_at`. Without the `id` key, LIMIT/OFFSET can repeat or drop a tree across pages.
- **Key note (the counter can leak via the shared endpoint):** `shared-family-tree.service.ts` returns `{ ...sharedFamilyTree, ...sharedFamilyTree.familyTree }`, spreading the whole tree row. `@ZodSerializerDto` strips the unknown key, and an E2E assertion now guards it. `visit_count` is deliberately absent from `FamilyTreeSchema`, so `getFamilyTreeById().visitCount` is also a compile error — tests read the row directly instead.
...
- Fixed by adding a scoped `overrides` entry to `biome.json` (`apps/api/src/**/*.ts` → `style.useImportType: "off"`) and deleting all 41 comments. `apps/web`/`libs/shared` keep the rule on — they don't use decorators.
- Modified: `biome.json`, 26 files under `apps/api/src/**` (comment deletions only, no logic changes).
- Verified: `pnpm biome check` clean, `nx run api:build:development` succeeds (confirms NestJS DI/decorator metadata still resolves at runtime with the rule off), `pnpm test:unit` all green (api 110, shared 225, web 76), `pnpm check:lint` clean across all 4 projects.
- Opened PR #519 (`chore/biome-import-type-503` → `develop`).
- **Key note (root cause, confirmed against Biome docs):** Biome's own `useImportType` documentation states it "doesn't know how a decorator is implemented" and can't detect that a type-only-looking import is also used as a value in decorator-generated code (NestJS/Angular DI, `emitDecoratorMetadata`). Biome's own recommendation for such codebases is to **disable the rule**, not suppress it per line — confirms the per-import `biome-ignore` pattern was fighting a rule that shouldn't have been enabled in `apps/api` to begin with.
- **Key note (git workflow gotcha — commit landed on local `develop`):** `/git-save` commits on whatever branch is currently checked out; this session had `develop` checked out (no feature branch created first), so the commit landed directly on local `develop` instead of a PR-able branch. Fix after the fact: `git checkout -b <feature-branch>` (carries the commit), then `git branch -f develop origin/develop` (resets local `develop` back to match remote — non-destructive since the commit still lives on the new branch), then push the feature branch and open the PR. **Lesson: create the feature branch BEFORE running `/git-save` when the work is meant to become a PR**, especially since local `develop` had drifted ahead of `origin/develop` with no feature branch to carry it.
