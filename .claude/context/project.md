...
- `biome.json` has an `overrides` block disabling `style.useImportType` for `apps/api/src/**/*.ts` — NestJS decorator metadata (DI, `@Query()`/`@Body()`/`@Param()` DTOs) needs these imports to stay as values at runtime, which Biome can't detect statically. Biome's own docs recommend disabling the rule entirely for decorator-heavy codebases rather than suppressing per-import. Scoped to `apps/api` only — `apps/web`/`libs/shared` don't use decorators and keep the rule on.
